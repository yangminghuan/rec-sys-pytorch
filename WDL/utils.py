...
class WDLDataset(Dataset):
    """
    定义WDL模型测试用的dataset
    """
    def __init__(self, df, sparse_feature):
        self.dense_input = df.drop(sparse_feature + ['label'], axis=1).values.astype('float32')
...
def test(model, test_df, sparse_feature):
    """
    用于模型的测试
    :param model: 训练完成的模型
    :param test_df: 测试数据
    ...
    """
    with torch.no_grad():
        test_dense = torch.from_numpy(test_df.drop(sparse_feature + ['label'], axis=1).values.astype('float32'))
